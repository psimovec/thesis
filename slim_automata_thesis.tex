...
\begin{document}
	...
	\chapter{Implementation}
		...
		Seminator is a Linux command-line tool which can be run with the \texttt{seminator} command. The tool transforms transition-based generalized B̈uchi automata (TGBAs) into equivalent semi-deterministic automata. \cite{seminator2}
...
\end{table}
	...
	\end{table}
		...
		\clearpage
		\section{----pryc Note about optimized automata}
	...
	\begin{table}
		\centering
		\caption{Table shows that ltl2ldba has the best results most often, but Seminator slim has some best hits too.}
	...
	\begin{figure}
		\centering
		\caption{Sorted values for each benchmark run for each tool.}
