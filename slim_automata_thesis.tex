...
\begin{document}
	...
	\chapter{Preliminaries}
		...
		\section{Büchi Automaton}
			...
			\paragraph{}
			A transition-based Büchi Automaton (TBA) is a quintuple $A=(\Sigma, Q, q_i, \Delta, \Gamma)$. An alphabet is a set of letters. $\Sigma$ is a finite \emph{alphabet} recognized by A. $Q$ is a finite set of \emph{states} of $A$, $q_0 \in Q$ is called the initial state of $A$. Transitions of $A$ directionally connect two states inside $Q$ with a letter from alphabet $\Sigma$. We write the set of \emph{transitions} as $\Delta \subseteq Q \times \Sigma \times Q$. Subset of the transitions $\Gamma \subseteq \Delta$ are \emph{accepting transitions}.
			\paragraph{}
