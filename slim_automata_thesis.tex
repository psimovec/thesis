...
\begin{document}
	...
	\chapter{Preliminaries}
		...
		\section{Büchi Automaton}
			...
			\paragraph{}
			A transition-based Büchi Automaton (TBA) is a quintuple $A=(\Sigma, Q, q_i, \Delta, \Gamma)$. An \emph{$\omega$-word} is an infinite sequence of letters. An alphabet is a set of letters. A \emph{language} is a set of $\omega$-words. $\Sigma$ is a finite \emph{alphabet}. $Q$ is a finite set of \emph{states} of $A$, $q_0 \in Q$ is the initial state of $A$. Intuitivelly, a transition $(s, a, t)$  directionally connects two states inside $Q$ with a letter from alphabet $\Sigma$. We write the set of \emph{transitions} as $\Delta \subseteq Q \times \Sigma \times Q$ and $\Gamma \subseteq \Delta$ is a set of \emph{accepting transitions}.
			\paragraph{}
			A \emph{Run} $r \in \Delta^\omega$ on A is an infinite sequence of transitions such that for the first transition $r_0=(q'_0, a, q''_0) \in r$ holds that $q'_0$ is the initial state of A and for each subsequent state $r_i=(q'_i, a, q''_i)$ holds that $q'_i=q''_{i-1}$
