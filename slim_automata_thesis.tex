...
\begin{document}
	...
	\chapter{Preliminaries}
		...
		Breakpoint automata (BPA) are a building block for slim automata. BPA are constructed from BA and are deterministic, but their language is only a subset of the language from original BA. 
		...
		Given the set of transitions, we define the function $\delta \colon 2^Q \times \Sigma \rightarrow 2^Q$ as 	
			$\delta\colon (S,a)\eqdef\{q'\in Q \mid \exists q \in S.(q,a,q') \in \Delta\}$ and we define $\gamma$ from $\Gamma$ analogously.
