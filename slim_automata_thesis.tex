...
\end{table}
	...
	Experiment uses benchmarks provided with the tool - examples/. The examples are built by various ways, some are even handcrafted. Using LTL that is provided we create different kinds of automata for comparison on benchmarks. Experiments are searching for lowest necessary amount of episodes needed for reaching probability 1 to hit the goal. Experiments run benchmarks 10 times with pseudo random seeds 0-10. If all runs ends with success, experiment computes median and average of results. Run can fail by timeout (600s) or by not reaching probability 1.
