...
\begin{document}
	...
	\chapter{Slim Automata Construction}
			...
			\begin{figure}[ht]
				...
				\input{slim.tikz}
				\end{center}
				\caption{Slim automaton (right) and the original Büchi automaton from Figure \ref{fig:bp:non-equivalent}(left)}
...
\end{table}
	...
	As Table \ref{table:seminator:optimized} shows, rom 4 base options; after applying post-optimizations strong slim algorithm surpasses weak one by resulting automaton size, even if it has worse results without the post-optimizations.
